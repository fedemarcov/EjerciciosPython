...
Por cada tanda de comida, se debe ingresar la cantidad de alimento en kg (número entero) que se le dio y se
le debe preguntar al animal si quiere seguir comiendo ('S', 'N').
Se desea conocer:
...
for comida in range (CANTIDAD_COMIDAS):
    print(f"Comida {comida+1} de {CANTIDAD_COMIDAS}")
    terminar = False
    while not terminar:
        kg = int(input("Kg: "))
...
print(f"Total: {total}")
print(f"Promedio por tanda: {total/CANTIDAD_COMIDAS}")
print(f"La comida donde mas comio fue la {comida_mayor} con: {mayor_kg} kg")